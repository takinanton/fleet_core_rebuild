...
# Env-file resolution (combo dir has NO systemd EnvironmentFile injecting DRY_RUN).
# Prefer .env.combo (the intended dual-leg, DRY_RUN=1 config) when present, else .env.
# Rationale (audit HIGH): config.py used to hardcode PROJECT_ROOT/.env. The loaded
# .env had DRY_RUN=0, and nothing ever read .env.combo -> a plain `python -m bot.main`
# would default to LIVE and place REAL orders on the unified HL account. ENV_FILE lets
# an operator override; otherwise .env.combo wins so the safe DRY config is loaded.
_ENV_OVERRIDE = os.getenv("ENV_FILE", "").strip()
if _ENV_OVERRIDE:
    _ENV_PATH = Path(_ENV_OVERRIDE).expanduser()
    if not _ENV_PATH.is_absolute():
        _ENV_PATH = PROJECT_ROOT / _ENV_PATH
elif (PROJECT_ROOT / ".env.combo").exists():
    _ENV_PATH = PROJECT_ROOT / ".env.combo"
else:
    _ENV_PATH = PROJECT_ROOT / ".env"
# Audit LOW (2026-06-19): override=True so the .env file's DRY_RUN WINS over any value
# already exported in the parent/process env. Without it, a stray `export DRY_RUN=0`
# would silently shadow the file's DRY_RUN=1 and the guard below (which only checked
# for None) would pass -> LIVE orders on the unified HL account.
load_dotenv(_ENV_PATH, override=True)

# ── Canonical DRY_RUN parse (SINGLE source of truth) ────────────────────────────────
# Audit MED (2026-06-19): the module guard and Settings.dry_run used to parse DRY_RUN
# with DIFFERENT rules. The guard only treated ("0","false","no","off") as LIVE, while
# _get_bool() returned DRY only for ("true","1","yes") and LIVE for EVERYTHING else.
# A truthy-looking typo -- DRY_RUN=on / 2 / dry / y / t / enabled -- slipped past the
# guard (no raise, no sign-off) yet resolved settings.dry_run=False -> the ENTIRE
# defense stack (config guard, trader._dry_block, adapter._dry_guard) keys off
# settings.dry_run, so ONE operator typo in the most safety-critical env var would
# silently arm REAL orders on the unified HL account.
#
# Fix: ONE canonical parser with EXPLICIT allow-lists. Anything outside both lists is
# rejected with a hard raise (fail-closed: an unrecognized value can never resolve to
# LIVE silently). Both the module guard below AND Settings.dry_run call this, so the
# two can never diverge again.
_DRY_TRUE_VALUES = ("1", "true", "yes", "on", "y", "t", "dry")          # -> DRY (safe)
_DRY_FALSE_VALUES = ("0", "false", "no", "off", "n", "f", "live")        # -> LIVE


def _parse_dry_run() -> bool:
    """Return True for DRY (safe), False for LIVE. Raise on missing/unrecognized.

    SINGLE source of truth for DRY_RUN. The module-level guard and Settings.dry_run
    both call this so a malformed value can never be DRY in one place and LIVE in
    another. Unrecognized -> hard raise (fail-closed), so no typo silently arms live.
    """
    raw = os.getenv("DRY_RUN")
    if raw is None:
        raise RuntimeError(
            f"DRY_RUN is not set in {_ENV_PATH} (or the process env). Refusing to "
            "start: the combo dir has no systemd EnvironmentFile, so an unset DRY_RUN "
            "would default to LIVE and place REAL orders. Set DRY_RUN=1 (safe) or "
            "DRY_RUN=0 (live, after sign-off) explicitly."
        )
    v = raw.strip().lower()
    if v in _DRY_TRUE_VALUES:
        return True
    if v in _DRY_FALSE_VALUES:
        return False
    raise RuntimeError(
        f"DRY_RUN={raw!r} is not a recognized boolean. Refusing to start (fail-closed: "
        "an unrecognized value must NEVER silently resolve to LIVE and arm real orders "
        f"on the unified HL account). Use one of {_DRY_TRUE_VALUES} (DRY/safe) or "
        f"{_DRY_FALSE_VALUES} (LIVE)."
    )


# Fail-closed DRY guard: refuse to start unless DRY_RUN is EXPLICITLY set AND recognized
# (a missing or malformed DRY_RUN raises inside _parse_dry_run). With no systemd
# injection in the combo dir this is the only line standing between `python -m bot.main`
# and real orders. To go live the operator must set DRY_RUN=0 explicitly (audit HIGH).
_DRY_IS_LIVE = not _parse_dry_run()

# Audit LOW (2026-06-19): going LIVE (DRY_RUN=0) requires an EXPLICIT sign-off flag
# (HL_LIVE_SIGNOFF=1) so an accidental/stale DRY_RUN=0 can never arm live trading.
if _DRY_IS_LIVE and os.getenv("HL_LIVE_SIGNOFF", "").strip() != "1":
    raise RuntimeError(
        f"DRY_RUN resolved to LIVE ({os.getenv('DRY_RUN')!r}) from {_ENV_PATH} / "
        "process env, but HL_LIVE_SIGNOFF=1 is not set. Refusing to arm live trading "
        "without explicit sign-off. Set HL_LIVE_SIGNOFF=1 to confirm, or DRY_RUN=1 to "
        "stay safe."
    )
...
class PerTFFilters:
    """Per-TF F1/F2/F3 filter overrides + donchian_k window."""
    f1: float
    f2: float
    f3: float
    donchian_k: int = 20
...
def _load_per_tf_filters(tf: str, global_f1: float, global_f2: float, global_f3: float, global_k: int = 20) -> PerTFFilters:
    tf_key = tf.upper()
    f1 = _get_float(f"TF_{tf_key}_F1", global_f1)
    f2 = _get_float(f"TF_{tf_key}_F2", global_f2)
    f3 = _get_float(f"TF_{tf_key}_F3", global_f3)
    k = _get_int(f"TF_{tf_key}_K", global_k)  # Donchian window; env TF_4H_K etc.
    return PerTFFilters(f1=f1, f2=f2, f3=f3, donchian_k=k)


@dataclass(frozen=True)
class PerTFShortFilters:
    """Per-TF SHORT F1/F2/F3 overrides + Donchian K for short direction.

    donchian_k: Donchian window for SHORT signal. Defaults to global DONCHIAN_K.
    Can differ from long K (e.g. 1h long K=20, 1h short K=40) per validated config.
    Env: TF_<TF>_SHORT_K (e.g. TF_1H_SHORT_K=40).
    """
    f1: float
    f2: float
    f3: float
...
def _load_per_tf_short_filters(tf: str, gf1: float, gf2: float, gf3: float, global_k: int = 20) -> PerTFShortFilters:
    tf_key = tf.upper()
    # TF_<TF>_SHORT_K: per-TF Donchian window for SHORT; falls back to global DONCHIAN_K.
    # Source: validated config requirement (e.g. 1h long K=20, short K=40 per sweep).
    short_k = _get_int(f"TF_{tf_key}_SHORT_K", global_k)
...
class Settings:
    ...
    # === Donchian / strategy ===
    zigzag_raw_length: int          # kept for backward compat (unused by Donchian entry)
    donchian_k: int                 # global default Donchian window (env DONCHIAN_K, default 20)
    raw_rr_target: float
    ...
    # === Trail-after-TP / structural-trail exit ===
    enable_trail_after_tp: bool
    trail_after_tp_buffer_pct: float   # also used as SL→breakeven buffer after partial TP
    trail_pivot_window: int
    max_run_r: float
    tp1_partial_frac: float            # fraction closed by the 50% reduce-only fib limit (0 = off)
    ...
    # Every bot SL must rest strictly INSIDE the liquidation price by this buffer:
    #   long  → SL_px > liqPx × (1 + buf)
    #   short → SL_px < liqPx × (1 - buf)
    # Mechanism: force-cross at entry; if venue forces isolated (thin HIP-3) and SL is
    # outside liq+buf → add isolated margin (REMEDY-A) ≤3 iters, else clamp SL (REMEDY-B).
    liq_sl_buffer_pct: float
    ...
    def from_env(cls) -> "Settings":
        ...
        global_donchian_k = _get_int("DONCHIAN_K", 20)
        ...
        for tf in _all_cfg_tfs:
            per_tf_filters[tf] = _load_per_tf_filters(tf, global_f1, global_f2, global_f3, global_donchian_k)
            per_tf_short_filters[tf] = _load_per_tf_short_filters(
                tf, short_global_f1, short_global_f2, short_global_f3, global_donchian_k
            )
        ...
        entry_limit_cap_pct = _get_float("ENTRY_LIMIT_CAP_PCT", 0.005)

        # Donchian window global default; per-TF override via TF_<TF>_K
        ...
        # COMBO-FIX (2026-06-19): creds are mandatory ONLY when NOT DRY. A DRY combo bot
        # (EXCHANGE=hyperliquid, DRY_RUN=1, blank account by design) places NO orders and
        # never signs, so requiring a populated agent key/address here would make
        # `python -m bot.main` hard-crash at import under the intended DRY config (and
        # would have FORCED a live wallet into .env.combo just to start). Gate on _dry_run
        # so the safe DRY config starts cred-less; going live (DRY_RUN=0) still hard-
        # requires the active adapter's creds.
        # Audit MED (2026-06-19): use the SAME canonical parser as the module guard so
        # _creds_required can never disagree with the module-level LIVE determination.
        _dry_run = _parse_dry_run()
        _creds_required = not _dry_run
        if exchange == "extended":
            ext_required = _creds_required
            pac_required = False
            hl_required = False
        elif exchange == "pacifica":
            ext_required = False
            pac_required = _creds_required
            hl_required = False
        elif exchange == "hyperliquid":
            ext_required = False
            pac_required = False
            hl_required = _creds_required
        else:
        ...
        return cls(
            ...
            # Audit MED (2026-06-19): reuse the canonical parser (NOT _get_bool, whose
            # rules diverged from the module guard) so settings.dry_run -- the value the
            # ENTIRE defense stack (trader._dry_block, adapter._dry_guard) keys off --
            # can never resolve LIVE while the module guard thought it was DRY.
            dry_run=_dry_run,
            ...
            max_run_r=_get_float("MAX_RUN_R", 1000.0),  # fail-safe default (S4-1 2026-06-20): validated combo = no R-cap; a dropped env key must NOT silently arm a +5R cap on either leg
            tp1_partial_frac=_get_float("TP1_PARTIAL_FRAC", 0.0),  # fail-safe default (donch-6 2026-06-20): dropped env key must NOT silently arm a non-v10d resting partial-TP+BE
...
# FOREIGN_SKIP_PREFIXES — comma-separated coin PREFIXES that are foreign-managed (e.g. the
# native-crypto positions owned by the xnn bot on the SHARED account). main.py parses the
# SAME env var for its adopt / untracked-protect fences; this module-level copy is the
# SINGLE SOURCE the orphan-sweep foreign-prefix fence reads (orphan_sweep._fenced).
# Previously only main.py defined it, so `from bot.config import FOREIGN_SKIP_PREFIXES`
# raised ImportError that the bare-except in orphan_sweep swallowed -> the fence was
# silently DEAD (audit FIX-5). Mirror main.py's os.getenv parse here verbatim.
FOREIGN_SKIP_PREFIXES: tuple = tuple(
    p.strip() for p in os.getenv("FOREIGN_SKIP_PREFIXES", "").split(",") if p.strip()
)

# MANUAL_POSITION_PREFIXES — comma-separated coin PREFIXES that are MANUAL user-managed
# positions on the SHARED HL account (e.g. manual ЮК xyz_ trades that carry NO trades.db
# row). Distinct from FOREIGN_SKIP_PREFIXES (stopped-bot positions) so the two operator
# decisions stay independent (see main.py def of the same name). Audit MEDIUM (2026-06-19):
# this lived ONLY in main.py, so orphan_sweep._fenced() (which reads bot.config) could not
# see it -> a pre-placed manual xyz_ reduce-only TRIGGER SL with no live position yet would
# be cancelled by the orphan-sweep after the 180s debounce = manual SL silently removed
# before fill. Same manual-exemption class FIX-5 fixed for FOREIGN_SKIP_PREFIXES; mirror
# main.py's os.getenv parse here verbatim so the sweep fence can read it.
MANUAL_POSITION_PREFIXES: tuple = tuple(
