...
def _restore_positions_from_db(client) -> Dict[str, Position]:
    ...
    for row in db_open:
        ...
        # 50/50 exit recovery: treat restored size as remainder baseline; SL price on
        # exchange == sl_current; resting fib-limit oid not tracked across restart
        # (it stays on the exchange as a harmless reduce-only order).
        pos.__dict__["_orig_size"] = row["size"]
