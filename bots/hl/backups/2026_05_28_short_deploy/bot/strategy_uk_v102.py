...
class Signal:
    coin: str
    tf: str
    side: str                    # always "long" (strategy is long-only)
    trigger_price: float         # ZigZag pivot high (breakout level)
    entry_price: float           # trigger + tick (stop-limit price)
    sl_price: float              # pivot low - tick
    tp1_price: float             # entry + 1.5R
    ...
    ema20: float
    f1_dist: float               # (close - ema20) / atr14
...
def scan_for_signal(
    ...
    L = zigzag_length
    last_pivot_high: Optional[tuple] = None  # (idx, price)
...
class PositionManager:
    ...
    def update_sl_on_new_bar(
        ...
        if enable_trail_after_tp and pos.tf in self.TRAIL_TP_TFS:
            if not pos.tp1_hit:
                # Check if TP1 just hit
                if high_i >= pos.tp1_price:
                    ...
                    pos.tp1_hit = True
                    log.info(
                        "TP1 HIT %s %s: trail activated, SL moved to %.6f",
                        pos.coin, pos.tf, new_sl,
                    )
                    return new_sl, None
            else:
                # TP already hit — check max_run_r cap and trail SL
                cur_r = (close_i - pos.entry_price) / sl_dist
                if cur_r >= self._max_run_r:
                    exit_px = pos.entry_price + self._max_run_r * sl_dist
                    log.info(
                        "MAX_RUN_R %.1fR cap: exit %s %s @ %.6f",
                        self._max_run_r, pos.coin, pos.tf, exit_px,
                    )
                    return None, "max_run_cap"

                # Trail: move SL up to recent_low - tick (source: uk_v84 trail pivot)
    ...
    def check_sl_hit(
        ...
        # Gap through SL (open below SL — fills at open)
        if o < sl:
            return (o, "gap_through_sl")

        # Wick touch SL (source: feedback_vstop_structure_wick_bug memory)
