...
def _restore_positions_from_db(client) -> Dict[str, Position]:
    ...
    for row in db_open:
        ...
        pos = Position(
            ...
            tp1_price=row["tp1"] or (row["entry"] + 1.5 * (row["entry"] - row["sl_initial"])),
            size=row["size"],
            bar_entry_idx=0,
        )
...
def main_loop(dry_run: bool = False, run_once: bool = False) -> None:
    ...
    position_manager = PositionManager(
        trail_after_tp_buffer_pct=settings.trail_after_tp_buffer_pct,
        trail_pivot_window=settings.trail_pivot_window,
