...
def manage_open_position(
    ...
    # SL "improves" = moves toward locking profit: UP for long, DOWN for short.
    is_long = (getattr(pos, "side", "long") == "long")
    sl_improved = (new_sl is not None) and (
        (is_long and new_sl > pos.sl_current) or
        ((not is_long) and new_sl < pos.sl_current)
    )
    if sl_improved:
        # Cancel old SL trigger and place new one (SHORT closes via BUY → is_buy=True)
        if sl_order_id:
