...
def _install_session_timeout(sess, what, connect=5.0, read=20.0):
    """ROOT-FIX class-guard 2026-07-02 (fleet propagation of the hung-fetch
    class): default hard (connect, read) timeout on EVERY request through a
    requests.Session, so a stalled socket fails fast instead of hanging the
    sync loop until the watchdog. Explicit per-call timeouts always win —
    the wrapper only fills timeout=None. Env: PACIFICA_SDK_READ_TIMEOUT."""
    try:
        read_to = float(os.getenv("PACIFICA_SDK_READ_TIMEOUT", str(read)))
    except Exception:
        read_to = read
    if sess is None:
        log.warning("%s has no session — cannot install HTTP timeout guard", what)
        return
    if getattr(sess, "_paci_timeout_installed", False):
        return
    ...
    sess._paci_timeout_installed = True
    log.info("SDK %s: default HTTP timeout installed (connect=%.0fs read=%.0fs)",
             what, connect, read_to)


def _sweep_untimed_sessions():
    """CLASS-GUARD 2026-07-02: SDK composition can hide embedded
    requests.Session objects. Walk live objects and guard ANY session without
    the flag — and WARN, so a new embedded client gets noticed instead of
    silently running unbounded."""
    import gc
    missed = 0
    for _obj in gc.get_objects():
        try:
            if isinstance(_obj, requests.Session) and not getattr(_obj, "_paci_timeout_installed", False):
...
def _fmt_decimal(value) -> str:
    """Serialize a size/price as a PLAIN fixed-point decimal string for the signed payload.

    Pacifica's signature/parser REJECTS scientific notation: ``str(1e-05)`` yields
    ``'1e-05'`` → HTTP 400 "Verification failed" (confirmed live 2026-06-01 on a 1e-05
    BTC dust close; sending ``'0.00001'`` returned 200). Any value whose default
    ``str()`` would use an exponent (very small or very large magnitudes) is reformatted
    to plain fixed-point: ``normalize()`` trims trailing-zero garbage and
    ``format(..., "f")`` guarantees no exponent. Values that already stringify cleanly
    pass through unchanged so known-good serializations are never perturbed.
    """
    s = str(value)
    if "e" in s or "E" in s:
        s = format(Decimal(s).normalize(), "f")
    return s
...
class PacificaClient:
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        # GLOBAL REST rate gate (RL-fix 2026-06-24): Pacifica ~10 req/s account-wide across
        # signed AND unsigned (/positions /orders /info/prices /kline /orderbook). Bar-close +
        # startup bursts 429'd. Pace EVERY session call through one lock (~7 req/s @140ms).
        self._rl_lock = threading.Lock()
        self._rl_last_ms = 0.0
        self._rl_min_interval_ms = float(os.getenv("PACIFICA_REST_MIN_INTERVAL_MS", "140"))
        _orig_request = self.session.request
        def _rl_gated_request(method, url, _orig=_orig_request, **kw):
            mi = self._rl_min_interval_ms
            if mi > 0:
                with self._rl_lock:
                    _n = time.time() * 1000.0
                    _w = mi - (_n - self._rl_last_ms)
                    if _w > 0:
                        time.sleep(_w / 1000.0)
                    self._rl_last_ms = time.time() * 1000.0
            return _orig(method, url, **kw)
        self.session.request = _rl_gated_request
        # Default timeout on the session (wraps the RL gate; explicit timeouts win)
        _install_session_timeout(self.session, "Pacifica-session")
        ...
        self._ws_feed = None  # WsCandleFeed attached by main_loop when PACIFICA_WS_CANDLE=1
    ...
    def _signed_request(
        ...
        # CLASS-GUARD (2026-06-22): order_type is NOT in Pacifica canonical create_order
        # signed schema; including it diverges signed data from server canonical -> HTTP 400
        # Verification failed. Strip defensively so any future builder is protected.
        if op_type == "create_order" and isinstance(payload, dict) and "order_type" in payload:
            log.warning("stripping spurious order_type from create_order payload")
            payload = {k: v for k, v in payload.items() if k != "order_type"}
        try:
    ...
    def account_value(self) -> float:
        ...
        if not resp.get("success"):
            ...
            return 0.0
        d = resp.get("data") or {}
        for k in ("account_equity", "cross_account_equity", "equity", "total_equity", "balance"):
            if k in d:
                try:
                    return float(d[k])
    ...
    def margin_used_usd(self) -> float:
        """REAL exchange initial-margin used, in USDC — account-level, all positions.

        Single source of truth for existing margin in the MM-cap gate. Returns the
        account-level `total_margin_used` from /account (live probe 2026-06-11:
        total_margin_used="1504.30"). This is the true cross-margin actually locked
        by ALL open positions (incl. manual/foreign), each on its OWN leverage — so
        it must NOT be reconstructed as Σ(notional)/leverage (that divides foreign
        positions by the new coin's leverage and is wrong, e.g. 20.3% modelled vs
        5.4% live).

        Per-position margin is unavailable: /positions returns margin_used absent and
        `value`=0, so summing per-position is impossible — account-level is the only
        truthful source.

        Raises RuntimeError if the /account request fails (fail-closed contract): the
        caller must NOT fall back to a notional/leverage model. A successful response
        reporting 0 margin (no positions) legitimately returns 0.0.
        """
        resp = self._signed_request("GET", "/account", "get_account", {})
        if not resp.get("success"):
            raise RuntimeError(f"margin_used_usd: /account failed: {resp.get('error', resp)}")
        d = resp.get("data") or {}
        if "total_margin_used" not in d:
            raise RuntimeError(f"margin_used_usd: total_margin_used missing in /account: {d}")
        try:
            return float(d["total_margin_used"] or 0)
        except (TypeError, ValueError) as e:
            raise RuntimeError(f"margin_used_usd: bad total_margin_used={d.get('total_margin_used')!r}: {e}")

    def open_positions(self) -> dict:
        """HL-shape positions dict. 5s TTL cache (mirror Extended).

        FAIL DIRECTION (audit 2026-07-02, phantom-guard mass-close class): a failed
        /positions read RAISES instead of returning {} or an arbitrarily-stale cache.
        The old {}-return made a 3-tick API outage look like "all positions gone" —
        the phantom-guard then auto-closed every DB row and cancelled live SLs; the
        stale-cache variant hid real closes for the outage duration. Callers must
        treat a raise as UNKNOWN (skip phantom counting / sweeps / entry), never
        as "absent".
        """
        ...
        if not resp.get("success"):
            log.error("open_positions failed: %s — raising (state UNKNOWN, not empty)",
                      resp.get("error", resp))
            raise RuntimeError(f"open_positions: /positions failed: {resp.get('error', resp)}")
    ...
    def candles(self, coin: str, interval: str, limit: int = 200) -> pd.DataFrame:
        ...
        # WS-PUSH fast path (2026-06-23): serve confirmed-final closed window from the live
        # candle feed -> 0 REST /kline. None unless warm+current -> REST fallback below.
        ...
        try:
            ...
            # XNN port 2026-06-11 (canon §0#10 class — exception-swallow fix): a failed
            # fetch must RAISE, not return an empty DataFrame the scanner/manage loop
            # silently treats as "no data, skip". All callers (scanner, main manage-loop,
            # liquidity_snapshot) wrap candles() in try/except and log loudly.
            raise RuntimeError(f"Pacifica /kline {coin} {interval} failed: {e}") from e
        ...
        df_full = df_full[["time", "Open", "High", "Low", "Close", "Volume"]].sort_values("time").reset_index(drop=True)
        _wsf = getattr(self, "_ws_feed", None)
        if _wsf is not None:
            try:
                _wsf.seed(coin, interval, df_full)
            except Exception:
                pass

        # XNN port 2026-06-11 (canon §0#10 candles-part): drop the in-progress bar —
        # CONDITIONALLY (class: mem:feedback_indexer_drop_last_assumes_forming). The old
        # code returned the raw response INCLUDING the forming bar, so the strategy's
        # "last closed bar" was actually live-updating intra-bar — guaranteed divergence
        # from bt (bt decides on closed bars only). Drop the last row ONLY if its ts is
        # inside the current forming bar (computed from TRUE now, not the jittered
        # cache-offset time) — an unconditional iloc[:-1] would cut the just-CLOSED bar
        # whenever the API hasn't yet emitted the forming one.
        _true_forming_start = (now_ms // bar_ms) * bar_ms
        _last_ts_ms = None
        if len(df_full) > 0:
            try:
                _last_ts_ms = int(pd.Timestamp(df_full["time"].iloc[-1]).value // 10**6)
            except Exception:
                _last_ts_ms = None
        ...
        # Cache freshness key = ts of the RAW last row (incl. forming bar) so the
        # bar-aligned cache check above still sees this fetch as current-bar fresh.
        last_bar_t_ms = current_bar_start
        if len(df_full) > 0:
            try:
                last_bar_t_ms = int(pd.Timestamp(df_full["time"].iloc[-1]).value // 10**6)
            except Exception:
                pass
        with self._cache_lock:
    ...
    def invalidate_candles_cache(self, coin: str, interval: str) -> None:
        """Drop the cached candles for (coin, interval) — scanner freshness-guard
        forced-refetch path (xnn port 2026-06-11)."""
        with self._cache_lock:
    ...
    def trigger_sl(self, coin: str, is_buy: bool, sz: float, trigger_px: float) -> dict:
        ...
        payload = {
            ...
            "reduce_only": True,
            "stop_order": {
                "stop_price": _fmt_decimal(trigger_px),
                "client_order_id": client_oid,
    ...
    def limit_reduce_only(self, coin: str, is_buy: bool, sz: float, px: float,
        ...
        """Resting reduce-only limit order for partial-TP.

        ROOT FIX 2026-06-10: previous impl delegated to trigger_tp → /orders/stop/create.
        That endpoint only handles stop-MARKET (triggers below mark for SL). A resting
        limit ABOVE mark (TP) returns "Invalid stop tick" from that endpoint.
        Correct path: /orders/create with order_type=limit, reduce_only=true, tif=GTC.
        Proven live: oid 9490704614 NEAR @2.366 posted via this path.

        GUARD: asserts this method NEVER calls /orders/stop/create or trigger_tp.
        Post-place read-back asserts order is resting (not filled, reduce_only=true).

        is_buy = CLOSE side (long → sell/ask side=False; short → buy/bid side=True).
        """
        # GUARD: class-level — this path must not use stop endpoint
        _CORRECT_ENDPOINT = "/orders/create"
        _CORRECT_TYPE = "limit"
        ...
        if sz <= 0:
            return self._to_error_resp(f"size <= 0 after lot rounding (coin={coin})")
        px = self.round_price(coin, px)
        if px <= 0:
            return self._to_error_resp(f"price <= 0 after tick rounding (coin={coin})")

        client_oid = str(uuid.uuid4())
        payload = {
            "symbol": coin,
            "side": "bid" if is_buy else "ask",
            "price": _fmt_decimal(px),
            "amount": _fmt_decimal(sz),
            "reduce_only": True,
            "tif": "GTC",
            "client_order_id": client_oid,
        ...
        log.info(
            "limit_reduce_only %s side=%s sz=%s px=%s (endpoint=%s type=%s)",
            coin, "buy" if is_buy else "sell", sz, px, _CORRECT_ENDPOINT, _CORRECT_TYPE,
        )

        resp = self._signed_request(
            "POST", _CORRECT_ENDPOINT, "create_order", payload,
        )
        if not resp.get("success"):
            err = resp.get("error") or str(resp)
            log.error("limit_reduce_only FAILED %s: %s", coin, err)
            return self._to_error_resp(err)

        d = resp.get("data") or {}
        oid = d.get("order_id") or d.get("id")
        if d.get("dry_run"):
            return self._to_resting_resp(oid or f"dry-{uuid.uuid4().hex[:8]}")
        if oid is None:
            return self._to_error_resp(
                f"limit_reduce_only: success but no order_id in response: {d!r}"
            )

        # POST-PLACE READ-BACK GUARD: verify order is resting & reduce_only (not filled/stop)
        try:
            rb = self.session.get(
                f"{REST_URL}/orders",
                params={"account": self.main_account, "limit": 20},
            ...
            rb_data = (rb.json() or {}).get("data") or [] if rb.status_code == 200 else []
            matched = next(
                (o for o in rb_data if str(o.get("order_id")) == str(oid)), None
            )
            if matched is not None:
                ot = (matched.get("order_type") or "").lower()
                ro = matched.get("reduce_only")
                filled = float(matched.get("filled_amount") or 0)
                if "stop" in ot:
                    raise AssertionError(
                        f"limit_reduce_only GUARD FAIL: order {oid} has order_type={ot!r} "
                        f"(stop endpoint leaked into limit path) — FAIL LOUD"
                    )
                if not ro:
                    raise AssertionError(
                        f"limit_reduce_only GUARD FAIL: order {oid} reduce_only={ro} "
                        f"(expected True) — FAIL LOUD"
                    )
                if filled > 0:
                    log.warning(
                        "limit_reduce_only read-back: order %s already partially filled "
                        "%.4f (price may have been at/past mark) — returning filled resp",
                        oid, filled,
                    )
                    avg_px = float(matched.get("average_filled_price") or px)
                    return self._to_filled_resp(avg_px, filled, oid)
                log.info(
                    "limit_reduce_only read-back OK: oid=%s type=%s reduce_only=%s filled=0",
                    oid, ot, ro,
                )
            else:
                log.warning(
                    "limit_reduce_only read-back: oid=%s not found in open orders "
                    "(may have filled immediately — acceptable)", oid,
                )
        except AssertionError:
            raise  # FAIL LOUD — propagate guard violations
        except Exception as e:
            log.warning("limit_reduce_only read-back probe failed (non-fatal): %s", e)
    ...
    def list_reduce_only_triggers(self) -> list[dict]:
        """ALL resting reduce-only TRIGGER (stop) orders, normalized for the
        orphan_sweep. RAISES on API failure (false-empty class fix) so the sweep
        skips the cycle rather than act on a maybe-empty view. reduce-only LIMIT
        take-profits (order_type 'limit', the manual ЮК TP shape) are intentionally
        excluded — they are not trigger orders. Cancel path: cancel_sl_order."""
        resp = self._signed_request("GET", "/orders", "get_orders", {})
        if not resp.get("success"):
            raise RuntimeError(
                f"list_reduce_only_triggers: GET /orders failed: "
    ...
    def list_open_sl_orders(self, coin: str) -> list:
        ...
        XNN port 2026-06-11 (canon §0#10): an API failure RAISES instead of returning []
        — `[]` is indistinguishable from "definitively no SL" and was the root of the
        «429→false-naked→дубль-SL/emergency-close» class. Callers must treat an
        exception as "can't prove it's gone" (assume-live), not as naked.
        """
        resp = self._signed_request("GET", "/orders", "get_orders", {})
        if not resp.get("success"):
            raise RuntimeError(
                f"list_open_sl_orders({coin}): GET /orders failed: "
                f"{str(resp.get('error', resp))[:200]}"
            )
        out = []
        for o in resp.get("data") or []:
    ...
    def position_liquidation(self, coin: str) -> Optional[dict]:
        """{liq_px, margin_mode, leverage} for the open position on `coin`, else None.

        REMEDY primitive for trader.ensure_sl_inside_liq (2026-06-11,
        feedback_sl_must_be_inside_liquidation). open_positions() hard-codes
        marginMode="cross"/liquidationPx may be coerced, so this reads the RAW
        /positions row to surface the TRUE isolated flag + numeric liquidation_price.

        Pacifica /positions row keys (live probe 2026-06-11): amount, created_at,
        entry_price, funding, isolated(bool), liquidation_price, margin, side, symbol,
        updated_at. The exchange returns a concrete numeric liquidation_price even on
        CROSS (account-level liq projected per coin), so liq_px is usually a number;
        margin_mode is derived from the `isolated` boolean.

        NOTE: Pacifica has NO add-isolated-margin endpoint in this adapter, so
        trader.ensure_sl_inside_liq REMEDY-A (add margin) is a no-op here and the guard
        falls back to REMEDY-B (clamp SL inside liq). See add_isolated_margin TODO below.
        """
        resp = self._signed_request("GET", "/positions", "get_positions", {})
        if not resp.get("success"):
            log.warning("position_liquidation(%s) read failed: %s", coin, resp.get("error", resp))
            return None
        for p in resp.get("data") or []:
            if p.get("symbol") != coin:
                continue
            try:
                if abs(float(p.get("amount", 0) or 0)) < 1e-12:
                    return None
            except (TypeError, ValueError):
                return None
            liq_raw = p.get("liquidation_price")
            try:
                liq_px = float(liq_raw) if liq_raw not in (None, "") else None
            except (TypeError, ValueError):
                liq_px = None
            is_iso = bool(p.get("isolated", False))
            try:
                lev_val = int(p.get("leverage", 1) or 1)
            except (TypeError, ValueError):
                lev_val = 1
            return {
                "liq_px": liq_px,
                "margin_mode": "isolated" if is_iso else "cross",
                "leverage": lev_val,
            }
        return None

    # REMEDY-A (add isolated margin to push liqPx past the SL) is NOT implementable on
    # Pacifica: no add-margin / update_isolated_margin endpoint exists in this adapter
    # (the /account/margin endpoint only toggles margin MODE, payload {"symbol","is_isolated"}).
    # TODO(2026-06-11): if Pacifica ships a deposit-isolated-margin endpoint, add
    #   def add_isolated_margin(self, coin, usd) -> Optional[dict]  (POST /account/margin/add or
    #   equivalent, verified live) so ensure_sl_inside_liq REMEDY-A engages; until then the guard
    #   GUARD: MUST round(float(usd), 6) BEFORE the API call. Raw float -> float_to_int
    #   reject killed HL REMEDY-A (DELL 2026-06-23, -$187). See hl tests/test_remedy_a_float_round.py
    #   uses REMEDY-B (clamp SL strictly inside liq+buffer) which is always correct.
