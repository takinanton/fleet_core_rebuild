...
def _lookup_real_close_px(client, pos):
    ...
    if acc <= 0:
        return None
    exit_px = num / acc
    sl = float(getattr(pos, "sl_initial", 0) or getattr(pos, "sl_current", 0) or 0)
    tp = float(getattr(pos, "tp1_price", 0) or 0)
    reason = "liq_or_manual"
    if sl > 0 and abs(exit_px - sl) / sl < 0.01:
        reason = "sl"
