...
def scan_for_signal(
    ...
    if tf in ("4h", "1d"):
        # ЮК pullback entry (uk_v110, validated 4h/1d ONLY — bt-1 A/B 2026-05-29).
        # Retrace 38-62% of impulse -> enter on break of correction high B; SL at C.
        # Fast TFs intentionally keep chase (pullback avgR negative on 15m/1h).
        ph = last_pivot_high[1]
        ph_idx = last_pivot_high[0]
        pl = last_pivot_low[1]
        rng = ph - pl
        if rng <= 0:
            return None
        zone_hi = ph - 0.382 * rng
        zone_lo = ph - 0.618 * rng
        low_seg = df["Low"].iloc[ph_idx + 1 : i + 1].values
        if len(low_seg) == 0:
            return None
        c_rel = int(np.argmin(low_seg))
        c_price = float(low_seg[c_rel])
        c_idx = ph_idx + 1 + c_rel
        if not (zone_lo <= c_price <= zone_hi):
            return None
        high_seg = df["High"].iloc[c_idx + 1 : i].values
        if len(high_seg) == 0:
            return None
        trigger = float(np.max(high_seg))
        if close_i <= trigger:
            return None
        entry = trigger + tick_size
        if open_i > entry:
            entry = open_i
        sl = c_price - tick_size
    else:
