...
def attempt_entry(
    ...
    manage/exit loop keeps running. INERT while the env var is unset.
    ...
    # --- Entry leg routing: side-aware cap (limit_px = fill-cap) ---
    # crypto (non-xyz_) leg: IMMEDIATE market-at-close entry (2026-06-23).
    # PRICE-GATE BYPASSED for crypto: the gate was killing fast Donchian breakouts
    # (price moved past limit_px in the ~1s between bar close and gate check => 0/N
    # fills STOP-entry skip: price never reclaimed trigger within 30s). A market
    # order with slippage-cap=entry_limit_cap_pct (1%) already bounds fill price;
    # cap-breach => emergency-close + 15min cooldown.
    # xyz_ leg (future): unchanged stop-limit path (n/a on Extended today).
    ...
    log.info(
        "ENTRY %s %s: trigger=%.6f entry=%.6f limit_cap=%.6f sl=%.6f tp1=%.6f size=%s",
    ...
    # All coins on Extended are native crypto perps (no xyz_ leg exists here).
    if _in_entry_cooldown(signal.coin):
        log.info(
            "Entry cooldown active for %s — skipping (recent post-fill abort)",
            signal.coin,
        )
        insert_rejected(
            coin=signal.coin, tf=signal.tf,
    ...
    # --- Pre-send breakout-validity guard (2026-06-25; root: stale 8h fill -4.94% below trigger) ---
    # limit_px caps only the adverse-HIGH side; a signal whose price retraced back
    # through the breakout level (bar_age within gate but breakout failed) would still
    # market-fill far on the favorable side = buying a dead breakout. Require mark still
    # on the breakout side, within the SAME cap band as limit_px. Fast breakouts
    # (mark>=trigger for long) pass on the first check = zero added latency.
    _mk_guard = client.mark_price(signal.coin) or 0.0
    if _mk_guard > 0:
        _lo = signal.entry_price * (1 - _cap)
        _hi = signal.entry_price * (1 + _cap)
        if (is_long and _mk_guard < _lo) or ((not is_long) and _mk_guard > _hi):
            _reject_and_log(
                signal,
                "breakout_invalidated: mark=%.6f outside cap band [%.6f,%.6f] "
                "(entry=%.6f cap=%.4f)" % (_mk_guard, _lo, _hi, signal.entry_price, _cap),
            )
            return None

    # write-db-row-PRE-order (phase0 2026-07-02, port of hl canon 2026-06-21): journal a
    # 'pending' row BEFORE the market order is sent, so a crash between send/fill and the
    # journal-promote below leaves a recoverable trace (main._reconcile_pending at startup
    # promotes it if a live position exists, else deletes it) instead of a naked untracked
    # position. Promoted to 'open' on fill; deleted on every no-fill / abort path below.
    # 'pending' is invisible to open_trades()/adopt → never affects trading.
    ...
    if fill_result is None:
        ...
        insert_rejected(
            ...
            entry_price=signal.entry_price,
            sl_price=signal.sl_price,
            reason="entry_market_unfilled: no fill or cap-breach",
            direction=signal.side,
        )
        return None
    ...
    if sl_order_id is not None and not _sl_inside_liq_ok(client, signal.coin, signal.side, sl_to_place):
        ...
        # Verified close (phase0 2026-07-02, port of hl canon): market_close() RETURNS an
        # ok-SHAPED error dict on SDK failure (never raises), so the old bare call treated
        # a failed close as success → naked untracked position. _ensure_flat re-checks the
        # live position, retries the close, and places a protective reduce-only SL on any
        # residual — confirmed-flat readback, never trust the response dict.
        if not _ensure_flat(client, signal.coin, is_buy_open=is_long,
                            known_filled_sz=filled_size):
            log.critical(
                "EMERGENCY CLOSE (invariant) FAILED for %s — residual/UNKNOWN after retries "
                "(protective SL attempted) — MANUAL INTERVENTION REQUIRED", signal.coin,
            )
    ...
    if sl_order_id is None:
        ...
        # Verified close (phase0 2026-07-02): see invariant-fail path above — a bare
        # market_close 'succeeds' on an ok-shaped error dict; _ensure_flat is the
        # confirmed-flat-or-protective-SL path (donor hl trader.py SL-fail-3x abort).
        if not _ensure_flat(client, signal.coin, is_buy_open=is_long,
                            known_filled_sz=filled_size):
            log.critical(
    ...
    # --- Journal --- write-db-row-PRE-order (phase0 2026-07-02): the row was written
    # 'pending' BEFORE the order send (above); PROMOTE it to 'open' now with the ACTUAL
    # fill. On a crash between send and HERE the pending row + main._reconcile_pending
    # (startup) recover the position (adopt heals SL) instead of leaving it naked.
    promote_pending(
        pending_id,
        entry=actual_entry,
        size=filled_size,
...
def _ensure_flat(client, coin: str, is_buy_open: bool, known_filled_sz: float = 0.0) -> bool:
    ...
    for _ in range(3):
        ...
        # market_close() RETURNS an ok-SHAPED error dict on SDK failure (never raises for
        # those) — surface it at ERROR so a failed close is never silent; the flat-readback
        # above is the real success criterion, never the response dict (phase0 2026-07-02).
        try:
            _resp = client.market_close(coin)
        except Exception as e:
            log.error("_ensure_flat %s: market_close raised: %s", coin, e)
            _resp = None
        try:
            _sts = (((_resp or {}).get("response") or {}).get("data") or {}).get("statuses") or []
            if _sts and "error" in _sts[0]:
                log.error("_ensure_flat %s: market_close returned ok-shaped ERROR dict: %s",
                          coin, _sts[0]["error"])
        except Exception:
            pass
        time.sleep(1.0)
    try:
        ...
        # Position-read failure = UNKNOWN, and UNKNOWN must NEVER read as 'flat'
        # (fail-direction law, phase0 2026-07-02): report NOT flat so the caller keeps
        # the trade DB-open / goes CRITICAL instead of declaring success blind.
        log.critical("_ensure_flat %s: final position read FAILED (%s) — state UNKNOWN, "
                     "NOT declaring flat", coin, e)
        return False
    if not pos:
        return True
    try:
...
def _market_fill_crypto(
    ...
    Replaces the STOP-LIMIT continuation gate for native-perp coins on Extended.
    Calls client.market_open (SDK slippage = settings.slippage = entry_limit_cap_pct = 1%)
    which fills only if the orderbook mid is within that pct of the send price (Extended SDK
    uses get_price_with_slippage to set the IOC limit price).

    Extended _to_hl_response wraps the SDK OpenOrderModel into HL-shape:
      {"response": {"data": {"statuses": [{"filled": {"avgPx": str, "totalSz": str}}]}}}
    so the same parse path used in _place_and_wait_fill applies verbatim.

    If fill price > limit_px (cap breach) or fill_ratio < min_fill_ratio:
      emergency-close via _ensure_flat + register abort cooldown (15min block).

    On unparseable response: confirm fill via open_positions (phantom-fill prevention).
    ...
    # Parse Extended _to_hl_response(kind="filled") shape:
    # {"status":"ok", "response": {"type":"order", "data": {"statuses": [{"filled": {"avgPx":..., "totalSz":...}}]}}}
    try:
        statuses = resp["response"]["data"]["statuses"]
        s = statuses[0] if statuses else {}
        if "error" in s:
            log.warning("CRYPTO-MARKET market_open(%s) error response: %s", coin, s["error"])
            return None
        if "filled" in s:
            ...
            # Cap-breach check (side-aware): reject fills outside 1% slippage window.
            # 0.1% tolerance for rounding (Extended SDK rounds agg_price to tick).
            ...
            if avg_px > 0 and _breach:
                log.warning(
                    ...
                    "for %s — emergency close + cooldown",
            ...
            # Slip vs signal close[i] ≈ limit_px / (1 + cap); positive = adverse
            _close_approx = limit_px / (1.0 + abs(actual_px / limit_px - 1.0) + 1e-12) if actual_px > 0 else limit_px
            log.info(
                "CRYPTO-MARKET FILL %s: avg=%.6f sz=%.6f (slip_vs_cap=%.4f%%)",
                coin, actual_px, filled_sz,
                100.0 * (actual_px - limit_px) / max(limit_px, 1e-12),
            )
            return actual_px, filled_sz
    except Exception as e:
        log.warning("CRYPTO-MARKET fill parse error for %s: %s", coin, e)

    # Unparseable response: confirm fill via open_positions (phantom-fill prevention).
    # A non-200 or SDK exception that still placed an order shows up in positions within ~5s.
    _confirmed_sz = 0.0
    for _ in range(4):
        try:
            client.invalidate_positions_cache()
...
def manage_open_position(
    ...
    def _close_on_reason(_reason):
        """Per-reason exit_px (mirrors combo bot / Nado v10d port)."""
        ...
        if _reason == "max_run_cap":
            ...
            # 4R take: use tp1_price; inert at TP_R_MULTIPLE=999 (unreachable).
    ...
    # The donchian staging PM promotes trail_{i-1} into pos.sl_current ONLY on the
    # first tick of a new bar -- pos.sl_current == bt-1 bar-i resolve level all bar.
...
def _lookup_real_close_px(client, pos, sl_oid=None, tp_oid=None):
    ...
    mark@phantom. One position per coin, so the most-recent close-side fills summing to pos.size
    ARE this position's close. Returns (exit_px, reason) or None (true phantom: no close fill).
    GAP-FIX (2026-06-23): Extended fills carry `side` ('B'/'A'), NOT a HL-style `dir` token, so
    the old `dir.startswith('Close')` filter matched NOTHING -> every real exit returned None and
    was mis-recorded as phantom@mark. Match the CLOSING side (opposite the entry) instead.
    ...
    OID-FIRST attribution (2026-07-01): match the closing fills' order ids against OUR resting
    SL/TP orders before the price-tolerance fallback — authoritative regardless of fill price."""
    try:
        fills = client.user_fills() or []
    except Exception:
        return None
    api = pos.coin.replace("xyz_", "xyz:") if str(pos.coin).startswith("xyz_") else pos.coin
    _close_side = "A" if getattr(pos, "side", "long") == "long" else "B"  # close = sell a long / buy a short
    cl = [f for f in fills if str(f.get("coin", "")) == api and str(f.get("side", "")) == _close_side]
    if not cl:
        return None
    ...
    for f in cl:
        ...
        if sz <= 0 or px <= 0:
            continue
        acc += sz; num += sz * px
        for _k in ("order_id", "oid"):  # order_id = parent order; oid = trade id (legacy key)
            _o = f.get(_k)
            if _o not in (None, ""):
                used_oids.add(str(_o))
        if "LIQUID" in str(f.get("trade_type", "")).upper():
            saw_liq = True
        if acc >= target * 0.95:
            break
    if acc <= 0:
