...
class Settings:
    ...
    # Mechanism: force-cross at entry (Extended SDK 1.4.x is cross-only anyway); if the
    # live per-position liqPx is still inside the SL → clamp SL inside liq+buf (REMEDY-B)
    # + LOUD warn. Extended has NO add-isolated-margin API (SDK is cross-only) so REMEDY-A
    # is a no-op here; on HL (canon) REMEDY-A adds isolated margin first.
...
# XNN patch 2026-06-11 (canon §0.4/§0.9 foreign-safety, adapted for ext):
# - FOREIGN_SKIP_PREFIXES: comma-sep coin prefixes that belong to OTHER bots —
#   adopt (main.py) and manage (trader.py) hard-skip them by code. ext has no xyz_*
#   class, so default empty; kept for canon parity.
# - FOREIGN_EXCLUDE_COINS: comma-sep EXACT coins held/managed by the old extended-bot
#   a/b on the SAME account. Merged into FX_EXCLUDE => excluded from universe (scan),
#   adopt, and manage. MUST be filled AT FLIP with the old bots' live coins (probe
#   live exchange, not DB) — see DEPLOY_CHECKLIST §6/§10.
FOREIGN_SKIP_PREFIXES: tuple = tuple(
    p.strip() for p in os.getenv("FOREIGN_SKIP_PREFIXES", "").split(",") if p.strip()
)
_FOREIGN_EXCLUDE_COINS: frozenset[str] = frozenset(
    c.strip() for c in os.getenv("FOREIGN_EXCLUDE_COINS", "").split(",") if c.strip()
)

FX_EXCLUDE: frozenset[str] = frozenset({
    # XNN 2026-06-11: stale UK-SIG-PAUSE coins (ONDO/NEAR/VIRTUAL) REMOVED — those were
    # UK-signal pauses of the OLD extended-bot, irrelevant to XNN; inheriting them
    # silently narrowed the XNN universe vs bt (bt traded them). BNB STAYS: a manual
    # short position is held on this account class — the bot must never scan/adopt/
    # manage it (mem: project_manual_bnb_short_2026_06_05). Coins held by the OLD
    # bots are excluded at flip via FOREIGN_EXCLUDE_COINS (checklist §6.3) + the
    # account_position_exists code guard in trader.attempt_entry.
    "BNB",  # manual position — hands off

    "EUR-USD", "GBP-USD", "JPY-USD", "CHF-USD",
    "EURUSD", "GBPUSD", "USDJPY",
    "EUR", "GBP", "JPY", "CHF", "AUD", "NZD", "CAD",
}) | _FOREIGN_EXCLUDE_COINS
