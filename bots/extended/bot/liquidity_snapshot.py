"""liquidity_snapshot.py — daily liquidity screen for extended_bot_v2.
...
Fail-safe: ошибка на одной монете → log error, continue; весь script НЕ
падает целиком.
...
def _orderbook_metrics(client, coin: str) -> tuple[Optional[float], Optional[float]]:
    """One snapshot of Extended orderbook → (spread_pct, depth_top20_usd).

    Calls SDK: markets_info.get_orderbook_snapshot(market_name=...).
    Errors on individual coin do not abort the whole snapshot run.
    """
    try:
        m = client._market(coin)
    except Exception as e:
        log.warning("market lookup %s failed: %s", coin, e)
        return None, None

    async def _go():
        return await client._client.markets_info.get_orderbook_snapshot(market_name=m.name)

    try:
        ob = client._bridge.run(_go(), timeout=10)
    except Exception as e:
    ...
    bid_list = getattr(data, "bid", None) or getattr(data, "bids", None) or []
    ask_list = getattr(data, "ask", None) or getattr(data, "asks", None) or []
    ...
    def _parse(levels):
        ...
        for b in levels:
            try:
                px = float(b.price if hasattr(b, "price") else b[0])
                qty = float(b.qty if hasattr(b, "qty") else b[1])
                if px > 0 and qty > 0:
    ...
    bids = sorted(_parse(bid_list), key=lambda x: -x[0])
    asks = sorted(_parse(ask_list), key=lambda x: x[0])
...
def fetch_one(client, coin: str):
    ...
    from the daily snapshot. Returns a LiquidityProfile or None on failure.
...
def main(force: bool = False) -> int:
    ...
    log.info("Building Extended client for snapshot…")
    client = ExtendedClient(settings)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Daily liquidity snapshot for extended_bot_v2")
