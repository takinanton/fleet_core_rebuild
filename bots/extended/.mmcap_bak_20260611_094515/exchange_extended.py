...
if _SDK_OK:
    ...
    # ============================================================
    # PATCH 2 (2026-05-25): WS confirmation timeout 5s → 30s
    # ============================================================
    ...
    # market_open(XAU) → TimeoutError in 5s → bot logs "FAILED after 20s
    # polling (no position appeared)" → 0 trades opened in 24h. WS event
    # arrives but >5s after order placement on Tokyo→client RTT under load.
    # Fix: replace create_and_place_order entirely, raise timeout to 30s.
    try:
        ...
        async def _patched_create_and_place_order(
            ...
            async with order_waiter.condition:
                await _asyncio_ws.gather(
                    placed_order_task,
                    _asyncio_ws.wait_for(order_waiter.condition.wait(), 30),  # PATCHED 5→30
                    return_exceptions=False,
                )
                open_model = waiters[order.id].open_order
                del waiters[order.id]
                if not open_model:
                    raise ValueError("No Fill or Placement received for order")
                return open_model

        _BTC.create_and_place_order = _patched_create_and_place_order
        log.warning("x10 SDK monkey-patch: create_and_place_order WS timeout 5s→30s (fixes Tokyo WS lag on XAU/illiquid)")
...
class ExtendedClient:
    ...
    def open_positions(self) -> dict:
        ...
        try:
            ...
            # Phantom-close guard (audit 2026-05-27): on SDK/network failure return
